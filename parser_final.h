...
void printstack(stack<string> &st,stack<string> &st1, ofstream &output)
{
    cout<<"-------------START-------------"<<endl;
    output<<"-------------START-------------"<<endl;
    while(!st.empty())
...
void parse(string terms[41], int pr,struct node* root, stack<string> st, stack<string> st1, int inc, string inp[1000], unordered_map<string, int> &indexer, unordered_map<string, string[50]> &table, vector<string> &productions,int checker, unordered_map<string, string[50]> &follows,string add,string prod,string adders[1000],int inc1,int search,int search1,int curr, ofstream &output)
{
...
void levelorder(struct node* root,queue<struct node*>q, ofstream &output)
{
	q.push(root);
	int cnt=1,cnt2=0;
	while(!q.empty())
