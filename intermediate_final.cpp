/* Group 58 intermediate code generator Code
...
void sdt(struct node* root)
{
    ...
    while(!q.empty())
    {
        ...
        if(temp->data=="for2")
        {
        	if(temp->adj[2]->data=="id" && temp->adj[3]->data=="=" && temp->adj[4]->data=="id" && temp->adj[5]->data=="arithmetic_operator" && temp->adj[6]->data=="num")
        	{
        		string id1=temp->adj[2]->adj[0]->data;
        		string id2=temp->adj[4]->adj[0]->data;
        		string op=temp->adj[5]->adj[0]->data;
        		string id3=temp->adj[6]->adj[0]->data;
...
int main()
{
	...
	string ids[1000];
	string nums[1000];
	ofstream output("intermediate_output6.txt");
	...
	string prod;
	string adders[1000];
	int inc1=0,search=0,search1=0,curr=0;
	parse(terms,0,root,st,st1,inc,inp,indexer,table,productions,checker,follows,add,prod,adders,inc1,search,search1,curr,output);
	if(checker==0)
	{
		string nxt="";
		int ct=0;
	...
	for(int i=0;i<quadruples.size();i++)
    {
        ...
        output<<temp.first<<" "<<temp.second.first<<" "<<temp.second.second.first<<" "<<temp.second.second.second<<"\n";
    }
	return 0;
}
